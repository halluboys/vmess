...
def _format_users_list() -> str:
    """
    Deduplicate admin users display.
    Show only: user_id, username, nomor XL, VIP.
    """
    db = _load_user_db()
    users = db.get("users", {}) or {}
    ...
    for chat_id_str, rec in users.items():
        ...
        item = {
            ...
            "vip": _human_vip(int(chat_id_str)) if str(chat_id_str).isdigit() else "OFF",
        ...
        if old is None or item["updated_at"] >= old["updated_at"]:
            if old:
                ...
                if item["number"] == "-" and old["number"] != "-":
                    item["number"] = old["number"]
                if item["user_id"] == "-" and old["user_id"] != "-":
                    item["user_id"] = old["user_id"]
            merged[key] = item
        else:
            if old["username"] == "-" and item["username"] != "-":
                old["username"] = item["username"]
            if old["number"] == "-" and item["number"] != "-":
                old["number"] = item["number"]
            if old["user_id"] == "-" and item["user_id"] != "-":
                old["user_id"] = item["user_id"]
            merged[key] = old

    rows = sorted(merged.values(), key=lambda x: x["updated_at"], reverse=True)
    if not rows:
    ...
    for item in rows[:300]:
        ...
        lines.append(
            f"- user_id: `{item['user_id']}` | {uname} | nomor: `{item['number']}` | VIP: *{item['vip']}*"
        )
    return "\n".join(lines)
...
def _create_backup_zip(target_dir: str = None) -> str:
    if target_dir is None:
        target_dir = tempfile.gettempdir()
    os.makedirs(target_dir, exist_ok=True)
    zip_path = os.path.join(target_dir, f"bot_esim_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.zip")
    files_to_backup = []
    for p in ["tg-users.json", "refresh-tokens.json", "transactions.json"]:
        full = os.path.join(os.getcwd(), p)
        if os.path.exists(full):
            files_to_backup.append((full, p))
    with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zf:
