...
async def cmd_grant(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("❌ Format hari salah. Contoh: /grant Aliaa210 30")
        return
    ...
    exp = _grant_vip(int(target_chat_key), days)
    db = _load_user_db()
    rec = (db.get("users", {}) or {}).get(str(target_chat_key), {}) or {}
...
async def cmd_revoke(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    _revoke_vip(int(target_chat_key))
    db = _load_user_db()
    rec = (db.get("users", {}) or {}).get(str(target_chat_key), {}) or {}
    disp_user_id = rec.get("tg_user_id") or rec.get("user_id") or target_chat_key
    disp_username = rec.get("tg_username") or rec.get("username") or "-"
...
def _delete_user_by_id(target_id: str) -> bool:
    ...
    target_id bisa berupa chat_id, user_id telegram, atau username.
    """
    db = _load_user_db()
    users = db.get("users", {}) or {}

    target_chat_key = _resolve_target_chat_key(target_id)
    if not target_chat_key:
        return False

    if target_chat_key in users:
        del users[target_chat_key]
        db["users"] = users
        _save_user_db(db)
        return True
    return False
