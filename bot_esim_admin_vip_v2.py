...
# Set ADMIN_IDS env to comma-separated Telegram user IDs, e.g. "12345,67890"
ADMIN_IDS = set()
try:
...
def _is_admin(update: Update) -> bool:
    try:
        return int(update.effective_user.id) in ADMIN_IDS
    except Exception:
        return False
...
def _menu_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("📩 Kirim ID saya ke admin", callback_data="m:sendid")],
    ])
...
async def _send_id_to_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Send user's Telegram ID + username to all admins."""
    ...
    number = _get_user_number(chat_id) if chat_id else None

    if not ADMIN_IDS:
        if update.callback_query:
            await update.callback_query.message.edit_text("⚠️ Admin belum dikonfigurasi.")
        else:
            await update.message.reply_text("⚠️ Admin belum dikonfigurasi.")
        return
...
async def cmd_users(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for chat_id_str, rec in users.items():
        ...
        lines.append(f"- chat_id: `{chat_id_str}` | nomor: `{num}` | VIP: *{vip}*")
        count += 1
        if count >= 200:
            break
