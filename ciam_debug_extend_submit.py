...
def _extract_subscriber_id(json_body: dict):
    if not isinstance(json_body, dict):
        return None

    return (
        json_body.get("subscriber_id")
        or json_body.get("subscriberId")
        or json_body.get("data", {}).get("subscriber_id")
        or json_body.get("data", {}).get("subscriberId")
        or json_body.get("result", {}).get("subscriber_id")
        or json_body.get("result", {}).get("subscriberId")
    )
...
def get_otp(contact: str) -> str:
    ...
    try:
        ...
        print("raw-bytes:", response.content[:120])
        print("response body:", repr(response.text[:200]))

        if "application/json" not in (response.headers.get("Content-Type") or ""):
            print("OTP response is not JSON; server likely returned encoded data")
            return None
        try:
        ...
        subscriber_id = _extract_subscriber_id(json_body)
        if subscriber_id:
            return subscriber_id

        print(json_body.get("error") or json_body.get("message") or "Subscriber ID not found in response")
        return None
    except Exception as e:
...
def extend_session(subscriber_id: str) -> str:
    ...
    print("extend_session subscriber_id:", subscriber_id)
    print("extend_session querystring:", querystring)
    try:
        response = requests.get(url, headers=headers, params=querystring, timeout=30)
        print("extend_session status:", response.status_code)
        print("extend_session body:", repr(response.text))
...
def submit_otp(
    ...
    ts_header = ts_gmt7_without_colon(now_gmt7)
    ...
    print("submit_otp contact_type:", contact_type)
    print("submit_otp contact:", contact)
    print("submit_otp final_contact:", final_contact)
    print("submit_otp code:", code)
    print("submit_otp payload:", payload)
    try:
        response = requests.post(url, data=payload, headers=headers, timeout=30)
        print("submit_otp status:", response.status_code)
        print("submit_otp body:", repr(response.text))
        try:
            json_body = response.json()
        except Exception as e:
            print("submit_otp JSON parse error:", e)
            return None
