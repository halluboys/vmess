...
async def on_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == "wait_msisdn":
        ...
        try:
            otp_request_result = get_otp(str(number))
            if not otp_request_result:
                context.user_data.pop("state", None)
                await update.message.reply_text("❌ Gagal meminta OTP. Coba lagi.", reply_markup=_back_kb())
                return
            context.user_data["otp_subscriber_id"] = otp_request_result
    ...
    if state == "wait_otp":
        ...
        try:
            ...
            if not result or "refresh_token" not in result:
                await update.message.reply_text("❌ OTP salah / login gagal. Cek log terminal untuk response submit_otp terbaru.", reply_markup=_back_kb())
                return
