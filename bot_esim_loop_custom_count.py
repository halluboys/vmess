...
DAILY_TX_LIMIT = int(os.getenv("DAILY_TX_LIMIT", "5"))
...
def _admin_only(chat_id: int, user_id: int) -> bool:
    return _is_admin(chat_id, user_id)

ADMIN_LOOP_FLAGS: Dict[int, bool] = {}
ADMIN_LOOP_TASKS: Dict[int, asyncio.Task] = {}
...
def _grant_vip(chat_id: int, days: int) -> int:
    ...
    # supaya user tetap muncul di daftar users walau metadata belum lengkap
...
def _menu_kb(is_admin: bool = False) -> InlineKeyboardMarkup:
    rows = [
        [
            ...
            InlineKeyboardButton("📦 Cek Paket", callback_data="m:packages"),
        ],
        [
    ...
    if is_admin:
        rows.extend([
            [
                InlineKeyboardButton("🛡️ 5GB 7DAY", callback_data="m:admin5gb7day"),
                InlineKeyboardButton("🔁 Loop 5GB", callback_data="m:adminloop5gb7day"),
            ],
            [
                InlineKeyboardButton("⛔ Stop Loop", callback_data="m:adminstoploop5gb7day"),
            ],
            [
...
def _users_menu_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [
            InlineKeyboardButton("ℹ️ Info User", callback_data="m:users_info"),
            InlineKeyboardButton("➕ Tambah VIP", callback_data="m:users_addvip"),
        ],
        [
            InlineKeyboardButton("➖ Kurangi VIP", callback_data="m:users_reducevip"),
            InlineKeyboardButton("🧹 Hapus User", callback_data="m:users_delete"),
        ],
        [
...
def _resolve_target_chat_key(target: str):
    """
    Cari key chat_id berdasarkan:
    - chat_id langsung
    - tg_user_id / user_id
    - tg_username / username (dengan atau tanpa @)
    """
    db = _load_user_db()
    users = db.get("users", {}) or {}
    needle = str(target).strip().lstrip("@").lower()
    if not needle:
        return None
    ...
    for cid, rec in users.items():
        ...
        if needle == tg_user_id or needle == tg_username:
            return str(cid)
    return None

def _change_vip_days_by_target(target: str, days_delta: int):
    """
    Tambah/kurangi VIP berdasarkan user_id/username/chat_id.
    Return tuple (ok, resolved_chat_key, display_user_id, display_username, new_days)
    """
    target_chat_key = _resolve_target_chat_key(target)
    if not target_chat_key:
        return False, None, None, None, None

    db = _load_user_db()
    db.setdefault("users", {})
    rec = db["users"].get(str(target_chat_key)) or {}
    if not isinstance(rec, dict):
        rec = {}

    exp = int(rec.get("expires_at") or 0)
    current_days = max(0, (exp - _now_ts()) // 86400)
    new_days = current_days + int(days_delta)
    if new_days < 0:
        new_days = 0

    rec["expires_at"] = _now_ts() + new_days * 86400 if new_days > 0 else 0
    rec["updated_at"] = _now_ts()
    if not rec.get("tg_user_id"):
        rec["tg_user_id"] = int(target_chat_key)
    if rec.get("tg_username") is None:
        rec["tg_username"] = ""

    db["users"][str(target_chat_key)] = rec
    _save_user_db(db)

    disp_user_id = rec.get("tg_user_id") or rec.get("user_id") or target_chat_key
    disp_username = rec.get("tg_username") or rec.get("username") or "-"
    return True, str(target_chat_key), str(disp_user_id), str(disp_username), int(new_days)
...
def _header(chat_id: int) -> str:
    ...
    try:
        ...
        pass
    used_today = _user_daily_tx_count(chat_id)
    remaining_limit = max(0, DAILY_TX_LIMIT - used_today)
    return (
        "📱 *ISI KUOTA ESIM*\n"
       ...
       f"\U0001F4CA *Limit hari ini:* `{used_today}/{DAILY_TX_LIMIT}` | *Sisa:* `{remaining_limit}`"
    )
...
async def _run_admin_loop_5gb(context: ContextTypes.DEFAULT_TYPE, chat_id: int, user, n: str, total_loop: int):
    label = "5GB 7DAY"
    uuid = "162d261b-05c7-450c-9b7a-1873b160140c"
    package_no = 1
    delay_seconds = 60
    success = 0
    failed = 0
    ...
    try:
        for i in range(1, total_loop + 1):
            if ADMIN_LOOP_FLAGS.get(chat_id):
                await context.bot.send_message(
                    chat_id=chat_id,
                    text=f"⛔ Looping {label} dihentikan manual pada loop {i}/{total_loop}.\n✅ Sukses: {success}\n❌ Gagal: {failed}",
            ...
            await context.bot.send_message(
                chat_id=chat_id,
                text=f"🔄 Loop {i}/{total_loop}: memproses {label}...",
                reply_markup=_back_kb(),
            )
            ...
            try:
                ...
                if not picked or not picked.get("option_code"):
                    raise ValueError(f"Paket #{package_no} tidak ditemukan.")
                ...
                if not token_confirmation:
                    raise ValueError("token_confirmation tidak ditemukan.")
                ...
                if pay.get("ok"):
                    ...
                    await _notify_admin_transaction(
                        context,
                        status="SUCCESS",
                        chat_id=chat_id,
                        user_id=user.id,
                    ...
                    await context.bot.send_message(
                        chat_id=chat_id,
                        text=f"✅ Loop {i}/{total_loop}: sukses {label}.",
                        reply_markup=_back_kb(),
                    )
                else:
                    failed += 1
                    detail = str(pay.get("detail", ""))[:500]
                    details.append(f"{i}. {detail}")
                    _log_transaction(
                        chat_id=chat_id,
                        user_id=user.id,
                        username=(user.username or ""),
                    ...
                    await context.bot.send_message(
                        chat_id=chat_id,
                        text=f"❌ Loop {i}/{total_loop}: gagal {label}.\nDetail: {detail}",
                        reply_markup=_back_kb(),
                    )
            except Exception as e:
                failed += 1
                ...
                await context.bot.send_message(
                    chat_id=chat_id,
                    text=f"❌ Loop {i}/{total_loop}: error {label}.\nDetail: {detail}",
                    reply_markup=_back_kb(),
                )

            if i < total_loop and not ADMIN_LOOP_FLAGS.get(chat_id):
                await context.bot.send_message(
                    chat_id=chat_id,
                    text=f"⏳ Menunggu 60 detik sebelum transaksi berikutnya...",
                    reply_markup=_back_kb(),
                )
                await asyncio.sleep(delay_seconds)

        summary = f"🏁 Looping {label} selesai.\nTotal: {total_loop}x\n✅ Sukses: {success}\n❌ Gagal: {failed}"
        if details:
            summary += "\n\nDetail gagal:\n" + "\n".join(details[:10])
        await context.bot.send_message(chat_id=chat_id, text=summary[:4000], reply_markup=_back_kb())
    finally:
        ADMIN_LOOP_FLAGS.pop(chat_id, None)
        ADMIN_LOOP_TASKS.pop(chat_id, None)
...
async def on_click(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "m:users":
        ...
        await q.message.edit_text(
            ...
            reply_markup=_users_menu_kb()
        )
        return

    if data == "m:users_info":
        if not _admin_only(chat_id, update.effective_user.id):
            await q.message.edit_text("❌ Kamu bukan admin.", reply_markup=_back_kb())
            return
        await q.message.edit_text("🔄 Mengambil daftar user...", reply_markup=_back_kb())
        await _send_long(q.message, _format_users_list(), reply_markup=_users_menu_kb())
        return

    if data == "m:users_addvip":
        if not _admin_only(chat_id, update.effective_user.id):
            await q.message.edit_text("❌ Kamu bukan admin.", reply_markup=_back_kb())
            return
        context.user_data["state"] = "wait_users_addvip"
        await q.message.edit_text(
            "➕ Tambah Durasi VIP\n\n"
            "Kirim dengan format:\n"
            "user_id +hari\natau\nusername +hari\n\n"
    ...
    if data == "m:users_reducevip":
        if not _admin_only(chat_id, update.effective_user.id):
            await q.message.edit_text("❌ Kamu bukan admin.", reply_markup=_back_kb())
            return
        context.user_data["state"] = "wait_users_reducevip"
        await q.message.edit_text(
            "➖ Kurangi Durasi VIP\n\n"
            "Kirim dengan format:\n"
            "user_id -hari\natau\nusername -hari\n\n"
    ...
    async def _dor_esim(uuid: str, label: str, package_no: int = 1, notify: bool = True):
        if notify:
        ...
        try:
            ...
            if not picked or not picked.get("option_code"):
                if notify:
                    await q.message.reply_text(f"❌ Paket #{package_no} tidak ditemukan.", reply_markup=_back_kb())
                return {"ok": False, "detail": f"Paket #{package_no} tidak ditemukan."}
            pkg = get_package(AuthInstance.api_key, tokens, picked["option_code"])
            token_confirmation = pkg.get("token_confirmation","")
            if not token_confirmation:
                if notify:
                    await q.message.reply_text("❌ token_confirmation tidak ditemukan.", reply_markup=_back_kb())
                return {"ok": False, "detail": "token_confirmation tidak ditemukan."}
            ...
            if pay["ok"]:
                ...
                if notify:
                    await q.message.reply_text(f"✅ *SUKSES* dor {label} (Silahkan Dicek).", parse_mode="Markdown", reply_markup=_back_kb())
                return {"ok": True, "detail": ""}
            else:
                detail = pay["detail"]
                _log_transaction(chat_id=chat_id, user_id=update.effective_user.id, username=(update.effective_user.username or ""), full_name=(update.effective_user.full_name or ""), number=str(n), package=label, status="FAILED", detail=str(detail))
                await _notify_admin_transaction(
                    context,
                    status="FAILED",
                    chat_id=chat_id,
            ...
            _log_transaction(chat_id=chat_id, user_id=update.effective_user.id, username=(update.effective_user.username or ""), full_name=(update.effective_user.full_name or ""), number=str(n), package=label, status="FAILED", detail=err)
            await _notify_admin_transaction(
                context,
                status="FAILED",
                chat_id=chat_id,
            ...
            if notify:
                await q.message.reply_text(f"❌ Error: {err}", reply_markup=_back_kb())
            return {"ok": False, "detail": err}

    async def _loop_dor_esim(uuid: str, label: str, package_no: int = 1, total_loop: int = 10, delay_seconds: int = 60):
        await q.message.edit_text(
            f"🔁 Looping {label} dimulai.\nTotal: {total_loop}x\nJeda: {delay_seconds} detik",
            reply_markup=_back_kb()
        )
        success = 0
        failed = 0
        ...
        for i in range(1, total_loop + 1):
            await q.message.reply_text(
                f"🔄 Loop {i}/{total_loop}: memproses {label}...",
                reply_markup=_back_kb()
            )
            result = await _dor_esim(uuid, label, package_no, notify=False)
            if result.get("ok"):
                success += 1
                await q.message.reply_text(
                    f"✅ Loop {i}/{total_loop}: sukses {label}.",
                    reply_markup=_back_kb()
                )
            else:
                failed += 1
                ...
                details.append(f"{i}. {detail}")
                await q.message.reply_text(
                    f"❌ Loop {i}/{total_loop}: gagal {label}.\nDetail: {detail}",
                    reply_markup=_back_kb()
                )

            if i < total_loop:
                await q.message.reply_text(
                    f"⏳ Menunggu {delay_seconds} detik sebelum transaksi berikutnya...",
                    reply_markup=_back_kb()
                )
                await asyncio.sleep(delay_seconds)
        ...
        if details:
            summary += "\n\nDetail gagal:\n" + "\n".join(details[:10])
        await q.message.reply_text(summary[:4000], reply_markup=_back_kb())

    # 3) eSIM 5GB
    if data == "m:esim5":
        if not _has_vip(chat_id) and not _is_admin(chat_id, update.effective_user.id):
            await q.message.edit_text("⛔ Fitur tembak paket hanya untuk VIP.\nKetik /vip untuk cek status.", reply_markup=_back_kb())
            return
        ok_limit, used_today = _can_do_transaction(chat_id)
        if not ok_limit and not _is_admin(chat_id, update.effective_user.id):
            await q.message.edit_text(
                f"⛔ Limit transaksi harian tercapai.\nDipakai: {used_today}/{DAILY_TX_LIMIT} hari ini.",
                reply_markup=_back_kb()
            )
            return
    ...
    if data == "m:esim10":
        ...
        if not ok_limit and not _is_admin(chat_id, update.effective_user.id):
            ...
            return
        await _dor_esim("162d261b-05c7-450c-9b7a-1873b160140c", "eSIM 10GB", 22)
        return
    ...
    if data == "m:esim10v2":
        ...
        if not ok_limit and not _is_admin(chat_id, update.effective_user.id):
            ...
            return
        await _dor_esim("162d261b-05c7-450c-9b7a-1873b160140c", "eSIM 10GB v2", 23)
        return

    # Admin only: 5GB 7DAY
    if data == "m:admin5gb7day":
        if not _is_admin(chat_id, update.effective_user.id):
            await q.message.edit_text("❌ Menu ini hanya untuk admin.", reply_markup=_back_kb())
            return
        await _dor_esim("162d261b-05c7-450c-9b7a-1873b160140c", "5GB 7DAY", 1)
        return

    if data == "m:adminloop5gb7day":
        if not _is_admin(chat_id, update.effective_user.id):
            await q.message.edit_text("❌ Menu ini hanya untuk admin.", reply_markup=_back_kb())
            return
        if chat_id in ADMIN_LOOP_TASKS:
            await q.message.edit_text("⚠️ Masih ada loop yang berjalan. Tekan Stop Loop dulu kalau mau ganti.", reply_markup=_back_kb())
            return
        context.user_data["state"] = "wait_admin_loop5gb_count"
        await q.message.edit_text(
            "🔁 Loop 5GB 7DAY\n\nMasukkan berapa kali looping.\nContoh: 5 atau 100\n\nJeda otomatis 60 detik per transaksi.\nKetik batal untuk membatalkan.",
            reply_markup=_back_kb()
        )
        return

    if data == "m:adminstoploop5gb7day":
        if not _is_admin(chat_id, update.effective_user.id):
            await q.message.edit_text("❌ Menu ini hanya untuk admin.", reply_markup=_back_kb())
            return
        task = ADMIN_LOOP_TASKS.get(chat_id)
        if not task:
            await q.message.edit_text("ℹ️ Tidak ada loop yang sedang berjalan.", reply_markup=_back_kb())
            return
        ADMIN_LOOP_FLAGS[chat_id] = True
        await q.message.edit_text("⛔ Perintah stop dikirim. Loop akan berhenti setelah transaksi aktif selesai / sebelum loop berikutnya.", reply_markup=_back_kb())
        return
...
async def on_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == "wait_admin_loop5gb_count":
        if not _is_admin(chat_id, update.effective_user.id):
            context.user_data.pop("state", None)
        ...
        if chat_id in ADMIN_LOOP_TASKS:
            context.user_data.pop("state", None)
            await update.message.reply_text("⚠️ Masih ada loop yang berjalan. Tekan Stop Loop dulu kalau mau ganti.", reply_markup=_back_kb())
            return
        try:
            total_loop = int(text)
            if total_loop <= 0:
            ...
            await update.message.reply_text("❌ Input harus angka bulat positif. Contoh: 5 atau 100", reply_markup=_back_kb())
            return

        ok, n, err = _ensure_active(chat_id)
        if not ok:
            context.user_data.pop("state", None)
            await update.message.reply_text(err, parse_mode="Markdown", reply_markup=_back_kb())
            return

        context.user_data.pop("state", None)
        ADMIN_LOOP_FLAGS[chat_id] = False
        ADMIN_LOOP_TASKS[chat_id] = asyncio.create_task(
            _run_admin_loop_5gb(context, chat_id, update.effective_user, str(n), total_loop)
        )
        await update.message.reply_text(
            f"✅ Looping 5GB 7DAY dimulai sebanyak {total_loop}x.\nJeda: 60 detik per transaksi.\nGunakan tombol Stop Loop untuk menghentikan.",
            reply_markup=_back_kb()
        )
        return
    ...
    if state == "wait_users_addvip":
        ...
        try:
            ...
            await update.message.reply_text("❌ Format salah. Contoh: 7165758792 +20", reply_markup=_back_kb())
            return

        ok, _, disp_user_id, disp_username, new_days = _change_vip_days_by_target(target, days)
        context.user_data.pop("state", None)
        if not ok:
            await update.message.reply_text("❌ User tidak ditemukan.", reply_markup=_back_kb())
            return
        await update.message.reply_text(
            f"✅ VIP ditambah\nusername: @{disp_username if disp_username != '-' else '-'}\nuserid: {disp_user_id}\nVIP sekarang: {new_days} hari",
            reply_markup=_back_kb()
        )
        return

    if state == "wait_users_reducevip":
        chat_id = update.effective_chat.id
        user_id = update.effective_user.id
        if not _admin_only(chat_id, user_id):
            context.user_data.pop("state", None)
        ...
        try:
            ...
            await update.message.reply_text("❌ Format salah. Contoh: 7165758792 -6", reply_markup=_back_kb())
            return
...
async def cmd_users(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not _admin_only(chat_id, user_id):
        ...
        return
    await _send_long(update.message, _format_users_list())
...
async def cmd_grant(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(context.args) < 2:
        await update.message.reply_text("Format: /grant <user_id/username/chat_id> <hari>\nContoh: /grant 788529678 30\nContoh: /grant Aliaa210 30")
        return
    ...
    try:
        ...
        await update.message.reply_text("❌ Format salah. Contoh: /grant Aliaa210 30")
        return

    ok, _, disp_user_id, disp_username, new_days = _change_vip_days_by_target(target, days)
    if not ok:
        await update.message.reply_text("❌ User tidak ditemukan. Gunakan user_id / username yang ada di menu Users.")
        return
...
async def cmd_revoke(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(context.args) < 1:
        await update.message.reply_text("Format: /revoke <user_id/username/chat_id>\nContoh: /revoke 788529678")
        return

    target = str(context.args[0]).strip()
    ok, _, disp_user_id, disp_username, _ = _change_vip_days_by_target(target, -999999)
    if not ok:
        await update.message.reply_text("❌ User tidak ditemukan. Gunakan user_id / username yang ada di menu Users.")
        return
...
# --- Anti Abuse / Admin Notification helpers ---
def _today_str() -> str:
    return datetime.now().strftime("%Y-%m-%d")

def _user_daily_tx_count(chat_id: int) -> int:
    data = _load_tx_log()
    ...
    for t in txs:
        try:
            if str(t.get("time", "")).startswith(today) and int(t.get("chat_id", 0) or 0) == int(chat_id):
                count += 1
        except Exception:
            continue
    return count

def _can_do_transaction(chat_id: int):
    used = _user_daily_tx_count(chat_id)
    if used >= DAILY_TX_LIMIT:
        return False, used
    return True, used

async def _notify_admin_transaction(context: ContextTypes.DEFAULT_TYPE, *, status: str, chat_id: int, user_id: int, username: str, number: str, package: str, detail: str = ""):
    if not ADMIN_IDS:
        return
    ...
    msg = (
        ...
        f"Nomor: {number or '-'}\n"
        f"Paket: {package}\n"
        f"Pemakaian hari ini: {_user_daily_tx_count(chat_id)}/{DAILY_TX_LIMIT}"
    )
    if detail:
        detail = str(detail)
        if len(detail) > 800:
...
def _format_users_list() -> str:
    ...
    Format konsisten dan mudah dicopy.
    """
    db = _load_user_db()
    users = db.get("users", {}) or {}
    ...
    for item in rows[:300]:
        ...
        lines.append(f"- {uname} | userid: {item['user_id']} | VIP: {item['vip']}")
    return "\n".join(lines)
...
async def cmd_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"- VIP aktif: *{vip_active}*\n"
        f"- Limit harian/user: *{DAILY_TX_LIMIT}*",
        parse_mode="Markdown"
    )
