...
def show_main_menu(profile):
    ...
    print("14. Redemables")
    print("15. AUTO BUY Package (Loop Bonus)")
    print("R. Register")
    print("N. Notifikasi")
...
def auto_buy_package_loop():
    ...
    TARGET_OPTION_NUMBER = 200
    DELAY_SECONDS = 660  # 11 menit

    print("Memulai AUTO BUY PACKAGE...")
    print(f"Family Code   : {FAMILY_CODE}")
    print(f"Nomor Paket   : {TARGET_OPTION_NUMBER}")
    print(f"Pembayaran    : Bonus (B)")
    print("Looping setiap 11 menit. Stop manual: CTRL + C")
    print("-------------------------------------------------------")

    while True:
        try:
            # 1. Ambil data family
            data = get_family(api_key, tokens, FAMILY_CODE, None, None)
            if not data:
                print("Gagal load family. Coba lagi.")
                time.sleep(5)
                continue
            ...
            # 2. Cari paket ke-200
            selected = next((p for p in packages if p["number"] == TARGET_OPTION_NUMBER), None)

            if not selected:
                print("Paket nomor 200 tidak ditemukan.")
                time.sleep(10)
                continue

            option_code = selected["code"]

            print(f"Membeli paket #{TARGET_OPTION_NUMBER}: {selected['name']} - {selected['price']}")

            # 3. Ambil detail package untuk token & ts
            pkg_detail = get_package(api_key, tokens, option_code)

            if not pkg_detail:
                print("Gagal load detail paket.")
                time.sleep(10)
                continue

            token_confirmation = pkg_detail["token_confirmation"]
            ts_to_sign = pkg_detail["timestamp"]
            price = pkg_detail["package_option"]["price"]
            option_name = pkg_detail["package_option"]["name"]

            # 4. Eksekusi pembelian BONUS (choice B)
            print("Eksekusi pembelian bonus (B)...")
            ...
            if res and res.get("status") == "SUCCESS":
                ...
                print(" Gagal membeli. Res:", res)

            print(f"Tidur 11 menit...")
            print("-------------------------------------------------------")
            time.sleep(DELAY_SECONDS)

        except KeyboardInterrupt:
            print("Auto buy dihentikan manual.")
            break
