...
def auto_buy_package_loop():
    ...
    print("AUTO BUY MODE")
    print("Loop setiap 11 menit - proses selalu dimulai ulang dari awal.")
    print("Stop manual: CTRL + C")
    print("-------------------------------------------------------")
    ...
    while True:
        try:
            ...
            if not tokens:
                print("Tidak ada token aktif. Login kembali.")
                time.sleep(5)
                continue

            print("\n--- Memulai ulang proses pembelian ---")

            # 1. STEP: LOAD FAMILY CODE (seperti input ulang)
            print("Mengambil data family...")
            data = get_family(api_key, tokens, FAMILY_CODE, None, None)

            if not data:
                print("Gagal load family, retry...")
                time.sleep(5)
                continue
            ...
            # Safety check
            if TARGET_OPTION_NUMBER > len(packages):
                print("Nomor paket tidak ditemukan.")
                time.sleep(5)
                continue

            selected = packages[TARGET_OPTION_NUMBER - 1]
            option_code = selected["code"]

            print(f"Paket #{TARGET_OPTION_NUMBER}: {selected['name']} - Rp{selected['price']}")

            # 2. STEP: GET PACKAGE DETAIL (seperti buka paket manual)
            ...
            name = detail["package_option"]["name"]

            print("Eksekusi pembelian bonus...")

            # 3. STEP: PEMBAYARAN BONUS B (seperti pilih 'b' manual)
            ...
            if res and res.get("status") == "SUCCESS":
                print(" Sukses membeli paket bonus!")
            else:
                print(" Gagal membeli:", res)

            print(f"Menunggu 11 menit sebelum mengulang proses dari awal...")
            print("-------------------------------------------------------")
