...
async def handle_phone_number_input(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    - 'login_menu' akan selalu meminta OTP.
    - 'switch_account_menu' akan mencoba mengaktifkan sesi; jika gagal, baru minta OTP.
    """
    user_id = update.effective_user.id
    logger.info(f"Function 'handle_phone_number_input' executed by user {user_id}.")
    ...
    logger.info(f"Processing phone number '{phone_number}' for state '{state}'.")
    ...
    status_message = await update.message.reply_text(f"🔄 Memproses nomor `{phone_number}`...", parse_mode='Markdown')

    # Alur 'Login' selalu meminta OTP baru
    if state == 'waiting_phone_number_login':
        logger.info(f"Login flow initiated for {phone_number}. Requesting OTP.")
    ...
    # Alur 'Cek Login / Ganti Akun'
        ...
        if not user_exists:
            logger.info(f"User {phone_number} does not exist in tokens. Requesting OTP for first login.")
            await status_message.delete()
            context.user_data['temp_phone'] = phone_number
            context.user_data['state'] = 'waiting_otp'
            await request_and_send_otp(update, phone_number)
        ...
        success = False
        try:
            logger.info(f"User {phone_number} exists. Attempting to activate session.")
            # Panggil fungsi yang menyebabkan error di dalam try-except
            success = AuthInstance.set_active_user(int(phone_number))
        except Exception as e:
            logger.error(f"Exception in AuthInstance.set_active_user for {phone_number}: {e}", exc_info=True)
            success = False # Pastikan 'success' adalah False jika terjadi exception

        if success:
            # Jika berhasil, lanjutkan seperti biasa
            logger.info(f"Successfully activated session for {phone_number}.")
            await status_message.edit_text(f"✅ Sesi untuk `{phone_number}` telah diaktifkan. Menampilkan menu...", parse_mode='Markdown')
            await asyncio.sleep(1)
            await status_message.delete()
            await show_main_menu(update, context)
        else:
            # Jika gagal (baik karena exception atau return False), minta OTP
            logger.warning(f"Failed to refresh session for {phone_number}. Redirecting to OTP flow.")
            await status_message.edit_text(
                f"⚠️ Sesi Anda untuk `{phone_number}` tidak dapat diaktifkan (kemungkinan kedaluwarsa).\n\n"
                f"Silakan login ulang dengan OTP.",
                parse_mode='Markdown'
            )
