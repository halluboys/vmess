...
def _menu_kb(is_admin: bool = False) -> InlineKeyboardMarkup:
    ...
    if is_admin:
        ...
        rows.append([InlineKeyboardButton("📢 Broadcast Msg", callback_data="m:broadcast")])
        rows.append([InlineKeyboardButton("💾 Backup/Restore", callback_data="m:backupmenu")])
        rows.append([InlineKeyboardButton("📊 Statistik", callback_data="m:stats")])
        rows.append([InlineKeyboardButton("📑 Transactions", callback_data="m:txmenu")])
    else:
...
async def cmd_users(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for cid, rec in users.items():
        ...
        lines.append(f"- chat_id: `{cid}` | user_id: `{rec.get('user_id','-')}` | username: `@{rec.get('username','-')}` | nomor: `{num}` | VIP: *{vip}*")
        count += 1
        if count >= 200:
            break
