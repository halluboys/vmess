...
def submit_otp(
    ...
    try:
        ...
        if not response.text or not response.text.strip():
            print("[Error submit_otp]: empty response body")
            return None

        content_type = (response.headers.get("Content-Type") or "").lower()
        if "json" not in content_type:
            print(f"[Error submit_otp]: non-JSON response content-type={content_type}")
            return None

        try:
            json_body = response.json()
        except Exception as e:
            print(f"[Error submit_otp]: invalid JSON response: {e}")
            return None

        print("submit_otp json:", json_body)

        if "error" in json_body:
            print(f"[Error submit_otp]: {json_body}")
            return {
                "_error": json_body.get("error"),
                "_error_description": json_body.get("error_description"),
                "_raw": json_body
            }
